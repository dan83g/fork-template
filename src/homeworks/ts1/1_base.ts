...
export const getColorContrastValue = (rgb: TRgb) => {
...
export const getNumberedArray = <T>(arr: T[]): { value: T; number: number }[] =>
  arr.map((value, index) => ({ value, number: index }));

export const toStringArray = <T>(arr: T[]): string[] => arr.map((value, index) => `${value}_${index}`);
